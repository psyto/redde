...
core/                 shared library (was copy-pasted into every surface)
  rpc.mjs             canonical Solana + EVM JSON-RPC client (timeout + backoff)
  solana.mjs          base58 + account-buffer decoders
...
once copied. New code imports from `core/`; the existing per-surface scripts keep
their inline copies until each is migrated (and verified against live RPC) one at
a time — a mechanical, low-risk follow-up, not a rewrite.
